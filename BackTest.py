...
def ChartTrade(data, trade=pd.DataFrame(), addp=[], v_enable=True):
    ...
    if trade.shape[0] > 0:
        trade1 = trade.copy()
        # 取出進場明細，透過時間索引將資料合併
        buy_order_trade = trade1[[2, 3]].set_index(2)
        buy_order_trade.columns = ['buy_order']
        buy_order_trade = buy_order_trade.drop_duplicates()
        # 取出出場明細，透過時間索引將資料合併
        buy_cover_trade = trade1[[4, 5]].set_index(4)
        buy_cover_trade.columns = ['buy_cover']
        buy_cover_trade = buy_cover_trade.drop_duplicates()
        # 將交易紀錄與K線資料彙整
        data1 = pd.concat([data1, buy_order_trade, buy_cover_trade], axis=1)
    ...
    # 設置圖表寬度和高度
    #figratio = (16, 9)  # 設定為16:9的寬高比
    figratio = (20, 9)
    figscale = 1.5      # 調整圖表大小
    
    # 繪製圖表，設定figratio和figscale來調整寬度
    mpf.plot(data1, addplot=addp, style=mstyle, type='candle', volume=v_enable, figratio=figratio, figscale=figscale)
